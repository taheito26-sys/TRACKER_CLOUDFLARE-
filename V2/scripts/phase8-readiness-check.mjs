...
const migrationProgressPath = path.resolve(__dirname, '..', 'MIGRATION_EXECUTION_STATUS.md');
...
async function fetchJson(url, headers = {}) {
  try {
    ...
    return { status: res.status, ok: res.ok, json, error: null };
  } catch (err) {
    return {
      status: 0,
      ok: false,
...
function endpointHeader(pathname, check) {
  if (check.error) {
    return `### ${pathname} (NETWORK ERROR)` + toBlock({ error: check.error });
  }
  return `### ${pathname} (HTTP ${check.status})` + toBlock(check.json);
}
...
async function run() {
  ...
  const report = `# Phase 8 Readiness Report\n\n` +
    ...
    `**${allPass ? 'PASS' : 'FAIL'}**\n\n` +
    `## Endpoint Evidence\n` +
    endpointHeader('/api/system/version', checks.version) + `\n` +
    endpointHeader('/api/system/health', checks.health) + `\n` +
    endpointHeader('/api/system/migrations', checks.migrations) + `\n` +
    endpointHeader('/api/system/kpi-parity', checks.kpiParity) + `\n` +
    endpointHeader('/api/system/cutover-readiness', checks.cutoverReadiness) + `\n` +
    endpointHeader('/api/system/reconciliation-summary', checks.reconciliationSummary) + `\n`;
...
run().catch((err) => {
  console.error('[phase8] fatal:', err.message || err);
  process.exit(1);
});
